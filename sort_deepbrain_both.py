...
def main(args):
    ...
    output_root = f'data/processed/experiment/{subject}'
    os.makedirs(output_root, exist_ok=True)

    print(f'{"*"*20} Processing {subject} {"*"*20}')

    recordings_folder = f'{output_root}/recordings'
    saline_paths = sorted(glob.glob(f'data/raw/{saline_date}_saline/{subject}*/{subject}-0*/*.rhd'))
    drug_paths = sorted(glob.glob(f'data/raw/{drug_date}_cocaine/{subject}*/{subject}-0*/*.rhd'))

    files_file = f'{output_root}/files.csv'
    if not os.path.isfile(files_file):
        files = []
        file_index = 0
        for recording_path in saline_paths + drug_paths:
            if 'saline' in recording_path:
                curated_trace_path = recording_path.replace(f'{saline_date}_saline', f'{saline_date}_saline/{saline_date}_saline-traces-curated').replace('.rhd', '.png') 
            else:
                curated_trace_path = recording_path.replace(f'{drug_date}_cocaine', f'{drug_date}_cocaine/{drug_date}_cocaine-traces-curated').replace('.rhd', '.png') 
    ...
    for unit_id in sorting.unit_ids:
        ...
        if not os.path.isfile(unit_plot_file):
            plot_unit(
                unit_id, waveform_extractors, files['file_index'].to_list(), 
                ['blue' if condition == 'before-cocaine' else 'red' if condition == 'after-cocaine' else 'orange' if condition == 'before-pbs' else 'green'
                    for condition in files['condition'].to_list()], 
                savepath=unit_plot_file
            )
