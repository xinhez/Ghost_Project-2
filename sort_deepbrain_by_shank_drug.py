...
def main(args):
    ...
    recording_paths = sorted(glob.glob(f'data/raw/{sortdate}_cocaine/{subject}*/{subject}-0*/*.rhd'))
    ...
    if not os.path.isfile(files_file):
        ...
        for recording_path in recording_paths:
            curated_trace_path = recording_path.replace(f'{sortdate}_cocaine', f'{sortdate}_cocaine/{sortdate}_cocaine-traces-curated').replace('.rhd', '.png') 
    ...
    n_file = len(files)
    all_recordings = [sc.load_extractor(f'{recordings_folder}/file{file_index}') for file_index in range(n_file)]

    for shank in (pbar := tqdm (range(channel_indices.shape[0]))):
    # for shank in (pbar := tqdm([2, 3])):
        recordings = [file_recording.channel_slice(channel_ids=file_recording.channel_ids[channel_indices[shank]]).set_probe(probe) for file_recording in all_recordings]
        recording = sc.concatenate_recordings(recordings)

        # traces_folder = f'{output_root}/traces/shank{shank}'
        # os.makedirs(traces_folder, exist_ok=True)
        # for file_index, file_recording in enumerate(recordings):
        #     pbar.set_description(f'{subject} shank {shank} plotting trace {file_index+1}/{len(recordings)}')
        #     trace_file = f'{traces_folder}/file{file_index}.png'
        #     if not os.path.isfile(trace_file):
        #         plt.figure(figsize=(20, 10))
        #         traces = file_recording.get_traces().T

        #         trace_gap = 75
        #         for row, channel_index in enumerate(channel_indices[shank]):
        #             plt.plot(traces[row] + trace_gap * row)
        #             plt.text(traces.shape[1], trace_gap * row, f'ch{channel_index}')
        #         plt.ylim(-50, 600)
        #         plt.savefig(trace_file)
        #         plt.clf()
        #         plt.close('all')

        sortings_folder = f'{output_root}/sortings-{threshold}/shank{shank}'
        if not os.path.isfile(f'{sortings_folder}/sorter_output/firings.npz'):
            ...
            for file_index, file_sorting in zip(files['file_index'].to_list(), sortings):
                se.NpzSortingExtractor.write_sorting(file_sorting, f'{sortings_folder}/sorter_output/file{file_index}.npz')

        sorting = se.NpzSortingExtractor(f'{sortings_folder}/sorter_output/firings.npz')
        sortings = [se.NpzSortingExtractor(f'{sortings_folder}/sorter_output/file{file_index}.npz') for file_index in files['file_index'].to_list()]

        waveforms_folder = f'{output_root}/waveforms-{threshold}/shank{shank}'        
        ...
        units_folder = f'{output_root}/units-{threshold}/shank{shank}'
        ...
        for unit_id in sorting.unit_ids:
            ...
            if not os.path.isfile(unit_plot_file):
                plot_unit(unit_id, waveform_extractors, files['file_index'].to_list(), ['blue' if condition == 'before-cocaine' else 'red' for condition in files['condition'].to_list()], savepath=unit_plot_file)
